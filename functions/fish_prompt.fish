...
function m -d 'List bookmarks, jump to directory in list with m <number>'
    ...
    if [ (count $argv) -eq 1 ] && [ $argv[1] -ge 0 ] && [ $argv[1] -lt $num_items ]
        ...
        for i in (seq $num_items)
            if [ $PWD = $bookmarks[$i] ]
                ...
                if [ (math \( $num_items - $i \) % 2) -eq 0 ]
                    set_color normal
                else
                    set_color $budspencer_colors[4]
                end
            end
            echo '▶ '(math $num_items - $i)\t$bookmarks[$i] | string replace -r "^$HOME" "~"
        end

        set last_item '-'(math $num_items - 1)
        echo -en $budspencer_cursors[1]
        set input_length (string length -- (math $num_items - 1))
        read -p 'echo -n (set_color -b $budspencer_colors[2] $budspencer_colors[10])" ⌘ Goto [0'$last_item'] "(set_color -b normal $budspencer_colors[2])" "(set_color $budspencer_colors[10])' -n $input_length dir_num

        if [ $dir_num -ge 0 ] && [ $dir_num -lt $num_items ]
            cd $bookmarks[(math $num_items - $dir_num)]
        end

        for i in (seq (math $num_items + 1))
            tput cuu1
        end
        tput cuu1
        echo -en "\e[J"
